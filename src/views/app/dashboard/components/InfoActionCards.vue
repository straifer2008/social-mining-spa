<template>
  <div class="info-action-cards">
    ...
    <G-Card
      v-if="referralCounters?.inactive || referralCounters?.new"
      class="card-typeA info-action-cards__item m-0"
    >
      <div class="card-typeA__body">
        <div class="card-typeA__top">
          <vue-feather class="card-typeA__top--icon" type="users"></vue-feather>
          <h6 class="card-typeA__top--text">
            {{ $t('infoActionCards.partners') }}
          </h6>
        </div>
        <div class="card-typeA__bottom">
          <span class="text-warning d-block">
            <b>{{ referralCounters.inactive }}</b> {{ $t('infoActionCards.inactive') }}
          </span>
          <span class="text-primary d-block">
            <b>{{ referralCounters.new }}</b> {{ $t('infoActionCards.new') }}
          </span>
        </div>
      </div>
    </G-Card>
