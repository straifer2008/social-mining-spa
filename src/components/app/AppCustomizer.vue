<template>
      ...
      <div class="tab-content" id="c-pills-tabContent">
        <div class="customizer-header">
          ...
          <button
            ...
            @click.prevent="showThemeConfigModal = true"
          >
            Configuration
          </button>
        </div>
        <div class="customizer-body custom-scrollbar">
          ...
          >
            ...
            <div
              class="tab-content sidebar-main-bg-setting"
              id="pills-tabContent"
            >
              <div
                class="tab-pane fade"
  ...
  <!-- <Dialog
    ...
    :modal="true"
    position="right"
    v-model:visible="showThemeConfigModal"
  >
    <template #header>
      <h5 class="headerTitle mb-0">Customizer configuration</h5>
    </template>
    <p>
      To replace our design with your desired theme. Please do configuration as
      mention
    </p>
    <p>
    ...
    <div>
      <pre class="mb-0">
        <code>
          <textarea :value="data" ref="layout" rows="1" :style="styleObject"></textarea>
          {{layout}}
        </code>
      </pre>
    </div>
    <template #footer>
      <button class="btn btn-primary" @click="copyText()">Copy Json</button>
    </template>
  </Dialog> -->
</template>
...
export default {
  ...
  computed: {
    ...
    layout() {
      const data = this.$store.getters.getLayoutData
      return data
    }
  },
  methods: {
    ...
    customizeLayoutType(val) {
      this.$store.dispatch('setLayoutType', val)
      this.layoutType = val
    },
    customizeSidebarType(val) {
      this.$store.dispatch('setCustomizeSidebarType', val)
    },
    ...
    customizeLightColorScheme(val) {
      this.lightColor = val
      this.$store.dispatch('setColorLightScheme', val)
    },
    customizeDarkColorScheme(val) {
      this.$store.dispatch('setColorDarkScheme', val)
    },
    copyText() {
      this.$refs.layout.select()
      document.execCommand('copy')
      this.$notice({ detail: 'Code Copied to clipboard' })
    }
  }
}
</script>
