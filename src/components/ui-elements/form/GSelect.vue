<template>
  ...
  >
    ...
    <div
      class="g-select__select form-control"
      :class="{
        focus: showList,
    ...
    >
      <input
        v-if="autocomplete"
        type="text"
        class="g-select__select--autocomplete"
      ...
      <svg
        ...
        width="20"
        height="20"
        class="g-select__select--clearable"
    ...
    <transition name="show-list" mode="out-in">
      <ul v-if="showList" class="g-select__list">
        <li
          v-for="(item, idx) in filteredItems"
          :key="idx"
          class="g-select__list--item"
        ...
        <li
          ...
          class="g-select__list--item no-data"
        >
          <span class="item-name">no data</span>
        </li>
      </ul>
    </transition>
    <div class="g-select__messages">
      <transition name="show-message" mode="out-in">
        <span v-if="errorMsg" class="g-select__messages--error">
          {{ errorMsg }}
        </span>
        <div
          v-else-if="(hint || $slots.hint) && !errorMsg"
          class="g-select__messages--hint"
        >
          <slot name="hint">
            {{ hint }}
          </slot>
...
export default {
  ...
  methods: {
    onSelect(item) {
      this.selectedValue = item[this.itemName]
